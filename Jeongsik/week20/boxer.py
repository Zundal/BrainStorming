...
승률이 동일한 복서의 번호들 중에서는 자신보다 
몸무게가 무거운 복서를 이긴 횟수가 많은 복서의 번호가 앞쪽으로 갑니다.
...
def solution(weights, head2head):
    ...
    for i in range(len(head2head)):
        print(i+1, '번 선수 몸무게 ',weights[i])
        for j in range(len(head2head)):
            print(j+1, '번 선수 몸무게 ',weights[j])
            print(i+1, '번 선수와 ', j+1, '번 선수의 대결 결과 ',head2head[i][j])
    answer = []
    return answer
